...
import { PostHogCoreStateless, PostHogMetrics, PostHogPersistedProperty, getEventUuid, isBlockedUA, isPlainObject, isPostHogFetchContentTooLargeError, minimizeFlagCalledEventProperties, raceWithTimeout, resolveMetricsConfig, safeSetTimeout, uuidv7 } from "@posthog/core";
...
import { resolveCaptureMode } from "./capture-v1/config.mjs";
import { AI_ROUTE, ANALYTICS_ROUTE, isLegacyOnlyEvent } from "./capture-v1/routing.mjs";
import { V1CaptureSender } from "./capture-v1/sender.mjs";
import { eventByteSize, partitionAiBatch } from "./ai-capture/batching.mjs";
import { AI_CAPTURE_ENDPOINT_PATH, AI_CAPTURE_ROUTE, AI_MAX_EVENT_BYTES } from "./ai-capture/routing.mjs";
...
class PostHogBackendClient extends PostHogCoreStateless {
    ...
    persistedQueueKeyForRoute(route) {
        if (route === AI_CAPTURE_ROUTE) return PostHogPersistedProperty.AiCaptureQueue;
    ...
    async sendAiSubBatch(batch, retryOptions) {
        try {
            ...
            if (!isPostHogFetchContentTooLargeError(err)) throw err;
            if (1 === batch.length) {
                const [event] = batch;
