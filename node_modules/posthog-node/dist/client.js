...
const config_js_namespaceObject = require("./capture-v1/config.js");
const routing_js_namespaceObject = require("./capture-v1/routing.js");
const sender_js_namespaceObject = require("./capture-v1/sender.js");
const batching_js_namespaceObject = require("./ai-capture/batching.js");
const external_ai_capture_routing_js_namespaceObject = require("./ai-capture/routing.js");
...
class PostHogBackendClient extends core_namespaceObject.PostHogCoreStateless {
    constructor(apiKey, options = {}){
        ...
        super(normalizedApiKey, normalizedOptions), this._memoryStorage = new external_storage_memory_js_namespaceObject.PostHogMemoryStorage(), this._aiCaptureRouteActive = false, this._minimalFlagCalledEvents = false;
        this.options = normalizedOptions;
        this.captureMode = (0, config_js_namespaceObject.resolveCaptureMode)();
    ...
    getQueueRouteKey(message) {
        return 'v1' === this.captureMode && (0, routing_js_namespaceObject.isLegacyOnlyEvent)(message) ? routing_js_namespaceObject.AI_ROUTE : routing_js_namespaceObject.ANALYTICS_ROUTE;
    }
    persistedQueueKeyForRoute(route) {
        if (route === external_ai_capture_routing_js_namespaceObject.AI_CAPTURE_ROUTE) return core_namespaceObject.PostHogPersistedProperty.AiCaptureQueue;
        return route === routing_js_namespaceObject.AI_ROUTE ? core_namespaceObject.PostHogPersistedProperty.AiQueue : core_namespaceObject.PostHogPersistedProperty.Queue;
    }
    getActiveQueueRoutes() {
        const routes = 'v1' === this.captureMode ? [
            routing_js_namespaceObject.ANALYTICS_ROUTE,
            routing_js_namespaceObject.AI_ROUTE
        ] : [
            routing_js_namespaceObject.ANALYTICS_ROUTE
        ];
        if (this._aiCaptureRouteActive) routes.push(external_ai_capture_routing_js_namespaceObject.AI_CAPTURE_ROUTE);
        return routes;
    }
    getBatchEndpointPath(route) {
        return route === external_ai_capture_routing_js_namespaceObject.AI_CAPTURE_ROUTE ? external_ai_capture_routing_js_namespaceObject.AI_CAPTURE_ENDPOINT_PATH : super.getBatchEndpointPath(route);
    }
    async sendBatch(batchMessages, retryOptions, route = routing_js_namespaceObject.ANALYTICS_ROUTE) {
        if (route === external_ai_capture_routing_js_namespaceObject.AI_CAPTURE_ROUTE) return this.sendAiCaptureBatch(batchMessages, retryOptions);
        if ('v1' !== this.captureMode || route === routing_js_namespaceObject.AI_ROUTE) return super.sendBatch(batchMessages, retryOptions, route);
    ...
    async sendAiCaptureBatch(batchMessages, retryOptions) {
        const { batches, dropped } = (0, batching_js_namespaceObject.partitionAiBatch)(batchMessages);
        for (const { event, bytes } of dropped){
            const message = `Event ${event} (${bytes} bytes) exceeds the ${external_ai_capture_routing_js_namespaceObject.AI_MAX_EVENT_BYTES / 1048576}MiB limit for ${external_ai_capture_routing_js_namespaceObject.AI_CAPTURE_ENDPOINT_PATH}, dropping.`;
    ...
    async sendAiSubBatch(batch, retryOptions) {
        try {
            await super.sendBatch(batch, retryOptions, external_ai_capture_routing_js_namespaceObject.AI_CAPTURE_ROUTE);
        } catch (err) {
            if (!(0, core_namespaceObject.isPostHogFetchContentTooLargeError)(err)) throw err;
            if (1 === batch.length) {
                const [event] = batch;
                const eventName = 'string' == typeof event.event ? event.event : 'unknown';
                const message = `Event ${eventName} (${(0, batching_js_namespaceObject.eventByteSize)(event)} bytes) was rejected with 413 by ${external_ai_capture_routing_js_namespaceObject.AI_CAPTURE_ENDPOINT_PATH} on its own, dropping.`;
                this._logger.error(message);
                this._events.emit('error', new Error(message));
                return;
            }
    ...
    getV1Sender() {
        if (!this._v1Sender) this._v1Sender = new sender_js_namespaceObject.V1CaptureSender({
            host: this.host,
            apiKey: this.apiKey,
    ...
    get metrics() {
        if (!this._metrics) this._metrics = new core_namespaceObject.PostHogMetrics(this, (0, core_namespaceObject.resolveMetricsConfig)(this.options.metrics), this._logger);
    ...
    captureAi(props) {
        ...
        const uuid = (0, core_namespaceObject.getEventUuid)(props.uuid, core_namespaceObject.uuidv7);
    ...
    _sendPreparedAiEvent(props, immediate) {
        ...
        return this._sendPreparedEvent('capture', props, immediate, void 0, external_ai_capture_routing_js_namespaceObject.AI_CAPTURE_ROUTE);
    }
            ...
            if (void 0 === flagsResponse) featureFlagError = external_types_js_namespaceObject.FeatureFlagError.UNKNOWN_ERROR;
                ...
                if (void 0 === flagDetail) errors.push(external_types_js_namespaceObject.FeatureFlagError.FLAG_MISSING);
                    ...
                    result = {
                        ...
                        variant: flagDetail.variant ?? void 0,
    ...
    async _shutdown(shutdownTimeoutMs) {
        ...
        if (this._metrics) {
            await (0, core_namespaceObject.raceWithTimeout)(this._metrics.flush().catch(()=>{}), Math.max(0, shutdownDeadlineMs - Date.now()));
            this._metrics.reset();
        }
        try {
    ...
    async _prepareEventMessage(props, options = {}) {
        ...
        const finalProperties = this._shouldSendMinimalFlagCalledEvent(event, mergedProperties) ? (0, core_namespaceObject.minimizeFlagCalledEventProperties)(mergedProperties) : mergedProperties;
